...
def solve_model(D, W, symmetry_break=False, knapsack=True):
            ...
            for j in range(D[i][0]):
                for k in range(nbT):
                    for jj in range(max(0, j - 1), j):
                        s.Add(sum(x[i][jj][kk] for kk in range(k + 1)) >= x[i][j][k])
                    for jj in range(j + 1, min(j + 2, D[i][0])):
                        s.Add(sum(x[i][jj][kk] for kk in range(k, nbT)) >= x[i][j][k])
    if knapsack:
    ...
    P2T = [
        [
            D[i][1],
            [k for j in range(D[i][0]) for k in range(nbT) if SolVal(x[i][j][k]) > 0],
        ]
        for i in range(nbC)
    ]
    T2P = [
